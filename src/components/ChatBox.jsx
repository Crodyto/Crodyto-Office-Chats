...
import { FiSend, FiInfo, FiArrowLeft, FiUsers, FiUser } from "react-icons/fi"; 
...
const ChatBox = ({ roomId, currentUserUid, setActiveRoomId }) => {
  ...
  // --- SYSTEM MESSAGE SENDER FUNCTION ---
  const sendSystemMessage = async (text) => {
    try {
      await addDoc(collection(db, `chatRooms/${roomId}/messages`), {
        text: text, 
        isSystemMessage: true, // Eta true thakle UI te holud dekhabe
  ...
  const handleClearChat = async () => {
    const confirm = window.confirm("Are you sure you want to clear this chat for yourself? The other person will still see the messages.");
    if (confirm) {
      await updateDoc(doc(db, "chatRooms", roomId), {
        [`joinedAt.${currentUserUid}`]: serverTimestamp()
      });
      setShowDetails(false);
    }
  };
  ...
  if (roomData) {
    if (roomData.type === "group") {
      ...
      defaultSubtitle = `${roomData.participants.length} members`;
      isGroup = true;
    } else {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div className="chat-header" onClick={() => setShowDetails(true)} style={{cursor: 'pointer'}}>
        ...
        <button className="btn-icon" style={{backgroundColor: '#e6f2ff', color: '#005cbf'}}>
